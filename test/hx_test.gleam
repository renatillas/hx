...
pub fn vals_test() {
  // Regular JSON value
  let json_value = json.object([#("name", json.string("John")), #("age", json.number(30.0))])
  
  hx.vals(json_value, False)
  |> assert_attribute("hx-vals", "{\"name\":\"John\",\"age\":30}")
  
  // JavaScript computed value
  hx.vals(json_value, True)
  |> assert_attribute("hx-vals", "js:{\"name\":\"John\",\"age\":30}")
}
